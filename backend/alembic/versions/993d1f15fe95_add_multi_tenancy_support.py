...
def downgrade() -> None:
    # Check which tables exist to determine if we're downgrading from fresh install or upgrade
    connection = op.get_bind()
    inspector = sa.inspect(connection)
    existing_tables = inspector.get_table_names()

    # Check if contents table has community_id column (upgrade scenario) or is new (fresh install)
    has_contents_with_old_schema = False
    has_channel_configs_with_old_schema = False

    if 'contents' in existing_tables:
        columns = [col['name'] for col in inspector.get_columns('contents')]
        # If it has community_id, it was created/modified by this migration
        has_contents_with_old_schema = 'community_id' in columns

    if 'channel_configs' in existing_tables:
        columns = [col['name'] for col in inspector.get_columns('channel_configs')]
        has_channel_configs_with_old_schema = 'community_id' in columns

    # ### commands auto generated by Alembic - please adjust! ###
    # Handle contents table downgrade
    if has_contents_with_old_schema:
        # Check if this was an upgrade (table existed before) or fresh install
        # If fresh install, just drop the table. If upgrade, remove new columns.
        contents_cols = inspector.get_columns('contents')
        # If table has only new schema columns, it was created fresh - drop it
        # Otherwise, remove the added columns
        with op.batch_alter_table('contents', schema=None) as batch_op:
            batch_op.drop_constraint('fk_contents_community', type_='foreignkey')
            batch_op.drop_constraint('fk_contents_creator', type_='foreignkey')
            ...
            # Only drop these columns if we're not dropping the table
            batch_op.drop_column('scheduled_publish_at')
            batch_op.drop_column('created_by_user_id')
            batch_op.drop_column('community_id')

    # Handle channel_configs table downgrade
    if has_channel_configs_with_old_schema:
        with op.batch_alter_table('channel_configs', schema=None) as batch_op:
            batch_op.drop_constraint('fk_channel_configs_community', type_='foreignkey')
            batch_op.drop_constraint('uq_community_channel', type_='unique')
            batch_op.drop_index(batch_op.f('ix_channel_configs_community_id'))
            batch_op.drop_column('community_id')

    # Drop new tables created by this migration
